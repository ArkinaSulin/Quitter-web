...
- **Spelljammer design closed — v8.1 FINAL** (see `.scratch/spelljammer-mod/spec.md` + HANDBOOK §17 + `.scratch/shipyard-formula/shipyard.csv` + `.scratch/ship-builder/spec.md`): ships are hero-like (full actions, full movement, no retaliation/morale/rout, no 5-attack cap — rate of fire = weapons × Fire Cycle × crew). Scenario settings: **sub-turn toggle** (OFF = ground combatants; ON = 5 segments / 1 action per hero), **environment** (Space | Atmosphere, **default Atmosphere**), **firing arcs**. Movement on the fly: **speed per turn**, a Helm action moves `speed/5` hexes and adjusts speed by **Accel = 18×sails/mass** toward the active cap (**Top Speed** 12/11/10/9, **Atmosphere Spd authored**, Overthrust +2 with skill check + subsystem-damage risk); **MC = turn capacity per GAME TURN** (band formula by speed — 1,2,3,2; civilians never exceed 2, heavy armor disqualifies, Wasp the only 4-turn ship, Tyrant hover device); differential caps = persistent `Vp − Vt` chase; **hit boxes** (1t = 1 box, only armor + hullR safe, BoxHP = 5×(1+armorFactor) round up, weapon anchors S=10/20 L=20/40 reinforced, per-instance pools, hullR 1t/1000gp = +25 Ship HP AND reinforces next-in-order, crew quarters = ceil(totalCrew/5), Ship HP = frame base + hullR×25, **DT flat 15**, repair kits); Jettison Heavy = cargo-ejector device (deploys 1-t space mines). Stations + crew reserve + **info war** (Tiny open, Small+ own station only, Command Bridge panel sees all, destroyed → blackout); **Officer Action pool = 1 base + bridges + Int >16** = panel + enable-only (redo / +1 result), no gambits. Heroes never auto-die → D&D VTT handoff. **Shipyard v8.1**: armor eats mass (MassCap × armorFactor 0/0.2/0.4/0.1/0.5), Plated cost ×2, hullR 1t/1000gp, crew quarters 1t/5 crew, **Fire Cycle (Rd)** replaces ROF, seed = author's 18-template file (Scorpion cargo 10, Gun Boat hullR-20 experiment). DB: all `ship_`-prefixed tables (future migrations 066+).), MC fully derived, keel removed, cost reflects capability. Boarding explicitly out of scope.

...
- **MC (maneuver class) = hexes to travel per 60° turn** — an integer, **LOWER = tighter turn = better**, from the band formula (see Shipyard components): the 1,2,3,4 inverted-U by speed. **60°/turn = `speed ÷ MC`** (1 decimal, higher = better) — the derived turning allowance per game turn. This **replaces the `speed/3` min-straight rule** — no hex-advance requirement between turns; MC and 60°/turn are the rule.
...
- **MC (hexes to travel per 60° turn)** at per-turn speed s: `tier = floor(mass/25)`; `center = clamp(round(activeTopSpeed × 0.65) − tier, 2, 8)`; `W = max(0, rudders − tier)`; `peak = clamp(rudders − floor(mass/45), 1, 3)`. **MC(s)** = 4 if mass<25 ∧ rudders≥2 ∧ |s−center|≤0.5 (rare premium — Wasp only, unladen); 3 if peak≥3 ∧ |s−center|≤W; 2 if |s−center|≤W+2; else 1. **60°/turn = `speed ÷ MC`** (1 decimal, higher = better). **Rudders widen the peak, mass shifts it down and narrows it**; load taxes maneuver (a laden freighter is clumsy, running light restores agility).
...
- Ship turning is governed by **MC** (hexes to travel per 60° turn, band formula by speed; 60°/turn = speed ÷ MC) — no MP charge, no `speed/3` commitment; the MC/60°-per-turn model is the rule.

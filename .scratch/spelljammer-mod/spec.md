...
- **Top Speed per frame**: Tiny 12 · Small 11 · Medium 10 · Large 9. **Atmosphere Spd** is authored per ship (given, not calculated). The scenario **Environment** setting (Space | Atmosphere, **default Space**) picks the active cap: `activeTopSpeed = IF(environment = "Space", TopSpeed, AtmosphereSpd)`. **Overthrust** = active + 2 via a Helm skill check; failure risks propulsion subsystem damage. **MC is unchanged by environment** (the turn-capacity curve just reads the current speed).
...
- **MC (hexes to travel per 60° turn)** at per-turn speed s uses **current mass incl. cargo**. `u = s/T`; `fill = rudders ÷ frameMassCap` (tons: Tiny 35 · Small 55 · Medium 80 · Large 100 — tiny/small reach full rudder benefit cheaply); **`u* = clamp(0.33 + 5.4·fill + 0.2·(25/mass − 0.5), 0.33, 0.6)`** (sweet spot); **`w = clamp(0.4 + 0.05·rudders, 0.45, 0.7)`** (width); **`TE_max = clamp(3·(25/mass)^0.7, 0.8, 3)`** (peak). **`TE(s) = TE_max·max(0, 1 − ((u − u*)/w)²)`** (parabola); **`MC(s) = max(1, round(s / max(0.5, TE(s))))`**; **`TE = speed ÷ MC`** (1 decimal). **Rudders push the sweet spot forward and widen it; lighter ships peak higher and further forward; load pulls the sweet spot back** (a laden freighter is clumsy, running light restores agility). **Crew** = `crew_count` (Components box) = the ship's crew complement (current crew on board); **minimum crew** = Σ component requirements (rounded up to a whole number); **quarters = ceil(crew_count/5)**. The **`ship_crews` roster** (named/unnamed crew, level + six stats, optional cost, null ≠ 0) is capped at `crew_count`. **Officer actions / game turn** = no bridge: `max(1, helmsman Int mod)`; with bridge: `max(4, 4 + captain Int mod)` (Int mods come from crew dropped on stations; default 0 in the builder).
...
- **Environment** setting: `Space | Atmosphere` (**default Space**) — picks whether the active speed cap is TopSpeed or AtmosphereSpd. MC unchanged.
...
- Scenario settings: `spelljammer_enabled`, `sub_turn_toggle`, `environment` (default Space), `firing_arcs`.

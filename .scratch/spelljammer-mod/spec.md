...
- **MC = turn capacity per GAME TURN** — number of 60° facing changes allowed per game turn (spent across the 5 segments, typically 1 turn per segment), from the band formula (see Shipyard components): the 1,2,3,2 inverted-U curve by speed. This **replaces the `speed/3` min-straight rule** — no hex-advance requirement between turns; the allowance is the rule.
...
- **MC = turn capacity per GAME TURN** at per-turn speed s: `tier = floor(mass/25)`; `center = clamp(round(activeTopSpeed × 0.65) − tier, 2, 8)`; `W = max(0, rudders − tier)`; `peak = clamp(rudders − floor(mass/45), 1, 3)`. **MC(s)** = 4 if mass<25 ∧ rudders≥2 ∧ |s−center|≤0.5 (rare premium — Wasp only, unladen); 3 if peak≥3 ∧ |s−center|≤W; 2 if |s−center|≤W+2; else 1. Turns per game turn = MC (spent across the 5 segments). **Rudders widen the peak, mass shifts it down and narrows it**; load taxes maneuver (a laden freighter is clumsy, running light restores agility).
...
- Ship turning is governed by **MC** (turn capacity per game turn, band formula by speed) — no MP charge, no `speed/3` commitment; the MC allowance is the rule.

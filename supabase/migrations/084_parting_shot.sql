-- 084: Parting shot (zone-of-control disengagement attack).
--
-- A unit that voluntarily moves OUT of a hostile kill zone provokes one free
-- parting attack from every formed hostile whose kill zone it is leaving. Each
-- unit may make at most ONE parting shot per turn — a persisted flag, sibling of
-- archer_reaction_used, cleared at the unit's own turn start.

ALTER TABLE units ADD COLUMN IF NOT EXISTS parting_shot_used BOOLEAN NOT NULL DEFAULT false;

-- Rebuild the unit-field allowlist with partingShotUsed (same list as 075 + the
-- new entry; unit_field_to_column is the single source for apply_substeps).

...
describe('mapEntities mappers', () => {
  it('parses a snake_case row into a MapEntity', () => {
    const row = {
      ...
      walls: { '0,0,0': { a: { moveCost: 3, meleeAc: 2 }, b: { block: true } }, 'bad': {} },
    ...
    expect(m.walls).toEqual({ '0,0,0': { a: { moveCost: 3, meleeAc: 2 }, b: { block: true } } }); // invalid keys dropped
  ...
  it('round-trips an entity to a row', () => {
    const row = mapEntityToRow({
      ...
      scale: 1, gridRadius: 12, terrainCosts: { '0,-1': 2 }, walls: { '0,0,0': { a: { block: true }, b: {} } }, hexEffects: [],
      createdAt: '', updatedAt: '',
    }, 'user-1');

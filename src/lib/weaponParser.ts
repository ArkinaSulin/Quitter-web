...
export interface Weapon {
  ...
  magicRadius: number; // area radius in feet (0 for non-area) — 25 ft = one hex radius
  reach: boolean;
 ...
 * Format: "Name,AttackBonus,TargetType,DamageDice,Range,MagicRadius,Reach,NoRetaliation,FreeAction,IgnoreAttackMultiplier,IsTwoHanded"
 * Example: "Longsword,5,single,1d8,1,0,false,false,false,false,false;Fireball,7,area,8d6,4,2,false,false,false,false,false"
 * Older 7/8/9/10-field strings (missing flags) parse with missing flags false.
 */
...
export function stringifyWeapons(weapons: Weapon[]): string {
  ...
  return weapons
    .map(w =>
      `${w.name},${w.attackBonus},${w.targetType},${w.damageDice},${w.range},${w.magicRadius},${w.reach},${w.noRetaliation},${w.freeAction},${w.ignoreAttackMultiplier},${w.isTwoHanded}`
    )
    .join(';');
}

...
export function mapTemplate(row: any): UnitTemplate {
  return {
    ...
    raceId: row.race_id || '',
    raceName: row.unit_races?.name || '',
    raceBaseHd: row.unit_races?.base_hd || null,
    raceIconUrl: raceIconFromName(row.unit_races?.name, row.unit_races?.icon_url),
    raceCanCharge: row.unit_races?.can_charge || false,
    mountCanCharge: row.unit_mounts?.can_charge || false,
    ...
    armorName: row.unit_armors?.name || '',
    ...
    mountName: row.unit_mounts?.name || '',
    movementPoints: row.movement_points || 3,
    aggressiveness: row.aggressiveness || 3,
    baseMorale: row.base_morale || 3,
    moraleBoost: row.morale_boost || 0,
    commandPursuitPermit: row.command_pursuit_permit ?? false,
    ...
    darkvision: Number(row.darkvision ?? row.unit_races?.darkvision ?? 0) || 0,
    str: row.str ?? 0,
    dex: row.dex ?? 0,
    con: row.con ?? 0,
    int: row.int ?? 0,
...
export function mapTemplateToRow(template: UnitTemplate) {
  ...
  return {
    ...
    morale_boost: template.moraleBoost || 0,
    command_pursuit_permit: template.commandPursuitPermit ?? false,
    ...
    ignore_morale_checks: template.ignoreMoraleChecks || false,

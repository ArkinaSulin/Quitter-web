...
export function useHexGrid({
  ...
  const hexes = useMemo(() => {
    ...
    for (let q = -gridRadius; q <= gridRadius; q++) {
      for (let r = -gridRadius; r <= gridRadius; r++) {
        const s = -q - r;
        if (Math.abs(s) <= gridRadius) result.push({ q, r, s });
      }
    }
    return result;
  }, [gridRadius]);
  ...
  const drawUnit = useCallback(
    (ctx: CanvasRenderingContext2D, unit: Unit, renderHex: Hex, isGhost: boolean = false, opacity: number = 1.0) => {
  ...
  const drawGrid = useCallback(
    (ctx: CanvasRenderingContext2D, width: number, height: number) => {
      ...
      for (const hex of hexes) {
        ...
        let stroke = '#2a2a4a';
        let lineWidth = 0.8;
        if (isDragTarget) { fill = 'rgba(255, 255, 200, 0.25)'; stroke = '#ffcc00'; lineWidth = 3; }
        else if (isHovered) { fill = 'rgba(255, 255, 255, 0.15)'; stroke = '#88ccff'; lineWidth = 2; }
      ...
      for (const unit of units) {
        ...
        const isGhost = isDragging && !!dragTargetHex && (dragTargetHex.q !== unit.hex.q || dragTargetHex.r !== unit.hex.r);
        if (isDragging && isGhost) drawUnit(ctx, unit, unit.hex, true, 0.5);
        drawUnit(ctx, unit, renderHex, false, isDragging ? 0.9 : 1.0);
      }
    },
    [hexes, size, zoom, offsetX, offsetY, drawHex, hoveredHex, dragTargetHex, draggingUnitId, units, drawUnit]
  );

  // --- Mouse handlers ---
  const getHexFromEvent = useCallback((e: React.MouseEvent<HTMLCanvasElement>): Hex | null => {
  ...
  const getUnitAtHex = useCallback((hex: Hex): Unit | undefined => {
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (hex) {
      setHoveredHex(hex);
      onHexHover?.(hex);
      if (draggingUnitId) setDragTargetHex(hex);
    } else {
      setHoveredHex(null);
      onHexHover?.(null);
    }

...
**Turn reset (End Turn):** when a group's turn starts, every non-deleted unit on a team in that group resets `movementPointsAvailable` and `actionsAvailable` — **units: 0 MP / 2 actions** (MP is materialized when a move converts an action, see below), **heroes: FULL MP / 5 actions** (their whole movement up front — `hero_actions_per_turn`). `attacksUsed` resets to 0 for non-hero units. Turn order cycles friendly → enemy → neutral, skipping empty groups; `turn_number` increments on a full cycle. See §6.10 `endTurn` / `src/lib/turnState.ts`.
...
- **Top Speed per frame**: Tiny 12 · Small 11 · Medium 10 · Large 9. **Atmosphere Spd** is authored per ship. The scenario **Environment** setting (Space | Atmosphere, **default Space**) picks the active cap; **the MC/TE curve is unchanged** (computed against the frame TopSpeed). **Overthrust** = active + 2 via a Helm skill check — failure risks propulsion subsystem damage; at overthrust speed turning collapses to 1.
...
- **MC (maneuver class) = hexes to travel per 60° turn** — an integer, **LOWER = tighter turn = better**. **TE (Turning Efficiency) = `speed ÷ MC`** — non-integer, **HIGHER = better**, shown to 1 decimal (the number of 60° turns per game turn). MC replaces the `speed/3` min-straight rule (formula in §17.4). Firing arcs are a per-scenario toggle (ON = book arcs, OFF = all 360°). The **Jettison device** (Large) ejects 1 t of cargo over time (creates difficult-terrain hexes, deploys 1-t space mines).
...
- **Armor eats mass capacity**: `armorMass = MassCap × armorFactor` (Wood 0, Plated 0.2, Metal 0.4, Ceramic 0.1, Stone 0.5). **Ship mass = armor + Σ components + loaded cargo**, budget = MassCap; **deck** (weapons + specials + crew quarters ×1 per 5; rigging is above-deck) is the co-equal budget. Overload on either = soft penalty. **Cargo = leftover Available Space** (1 t capacity = 1 t cargo; **Cargo Area** = designated load, **Unclaimed Space** = Available − Cargo Area). Component masses: helm/aux 2, sail 2, rudder 2, L.Weap 4, S.Weap 2, **hullR 1** (1,000 gp, safe, +25 Ship HP and reinforces next-in-order), **crew quarters = ceil(crew_count/5) whole tons** (houses the crew you carry), bridge 2.
- **Accel = 18 × sails ÷ mass** toward the active speed cap; laden uses mass + cargo.
- **MC (hexes to travel per 60° turn)** at per-turn speed `s` uses **current mass incl. cargo** (laden = empty + cargo area). `u = s/T` (T = frame Top Speed); `fill = rudders ÷ frameMassCap` (tons: Tiny 35, Small 55, Medium 80, Large 100 — tiny/small reach full rudder benefit cheaply). **Sweet spot** `u* = clamp(0.33 + 5.4·fill + 0.2·(25/mass − 0.5), 0.33, 0.6)`; **width** `w = clamp(0.4 + 0.05·rudders, 0.45, 0.7)`; **peak** `TE_max = clamp(3·(25/mass)^0.7, 0.8, 3)`. **`TE(s) = TE_max·max(0, 1 − ((u − u*)/w)²)`** (a parabola — 0 at rest, peaks `TE_max` at `u*`, trails both ends); **`MC(s) = max(1, round(s / max(0.5, TE(s))))`**; **`TE = speed ÷ MC`** (1 decimal, higher = better). Rudders push the sweet spot forward and widen it; lighter ships peak higher and further forward; loading pulls the sweet spot back (a laden freighter is clumsy). **Reinforcement order** (each hullR doubles the next): Helm→Bridge→extra Helm/Bridge→L.Weap→Rudder→S.Weap, skipping missing; sails/cargo/unclaimed never reinforced.
- **Crew**: **`crew_count`** = the ship's crew complement (current crew on board), set in the Components box ("Crew"). **Minimum crew** = ceil(Σ component + accessory crew requirements) — helm 1, aux/rudder/L.Weap/S.Weap 1 each, sails 0.5 each, Command Bridge 2. **Crew quarters = `ceil(crew_count / 5)`** — they house the crew you actually carry. The **`ship_crews` roster** (named or unnamed crew with level + six stats + optional cost; null cost ≠ 0 = free) is capped at `crew_count`. **Officer actions / game turn** = no Command Bridge: `max(1, helmsman Int modifier)`; with a bridge: `max(4, 4 + captain Int modifier)` — Int modifiers come from crew dropped onto stations on the map (default 0 in the builder until that's live).
...
- **Officer Action pool** per game turn: **no Command Bridge** → `max(1, helmsman Int modifier)` (the helmsman counts as an officer); **with a Command Bridge** → `max(4, 4 + captain Int modifier)`. Int modifiers come from crew dropped onto the Helm/Captain stations on the map (defaults to 0 in the builder until crew drops are live). Low Int just **shrinks the pool** — no fail checks, no lost orders, no demoralizing probability.
...
- **Scheduled Maintenance**: rudder responsiveness +1 (treat as +1 rudder for the MC/TE band width `w`).
...
All spelljammer rules live in dedicated modules (see spec): `src/lib/shipMoveCost.ts`, `src/lib/shipCombat.ts`, `src/lib/shipStats.ts`, `src/types/ship.ts` + `shipMappers.ts`, `src/hooks/useShipEngine.ts`, `src/components/ScenarioMap/ShipPanel.tsx`, `src/components/ShipEditor/` (builder), `src/components/ShipRenderer/` (shared ship visuals — hit grid + functional-area board used by both the builder and the map). Tables (all `ship_`-prefixed): `ship_frames`, `ship_armors`, `ship_components`, `ship_accessories`, `ship_weapons`, `ship_templates` (+ accessory/weapon joins), `spelljammer_ships`. Scenario settings: `sub_turn_toggle`, `environment` (default Space), `firing_arcs`. The random box pick rides the command log so undo restores the identical outcome.

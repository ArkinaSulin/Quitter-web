...
- **Crew roster** (`ship_crews`): each template carries a roster of named or unnamed crew (one row per crew member) with level + the six stats (Str/Dex/Con/Int/Wis/Cha) and an optional cost (null = no listed cost, distinct from 0 = free). **Current crew** = roster count; **minimum crew** = Σ component crew requirements. **Officer actions / game turn** = no Command Bridge: `max(1, helmsman Int modifier)`; with a bridge: `max(4, 4 + captain Int modifier)` — the helmsman and captain each count as officers. The Int bonus comes from crew dropped onto stations on the map (default 0 in the builder until that's live).
